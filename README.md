...
- **ELM327 UUID 差异**：不同 ELM327 蓝牙版的 GATT 特征 UUID 不统一。代码默认尝试最常见的
  `0000ffe0/ffe1` 与 `0000fff0/fff2` 两组；若连上后 `onData` 一直无数据，在
  `BleObdManager.onServicesDiscovered` 打印 `svc.uuid` 即可定位，按需补充 UUID。
